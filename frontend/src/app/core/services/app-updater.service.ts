...
// Đã xoá switchMap, zip vì không cần xử lý async translation nữa
import { concat, filter, first, interval, map, mergeMap } from 'rxjs';

import { SeoService } from '@app/shared/services';
import { environment } from '@env/environment';

// Đã xoá import I18nService
...
export class AppUpdater {
  ...
  constructor(
    ...
    // Đã xoá i18nService trong constructor
  ...
  public handleRouterChangesAndUpdateMeta(): void {
    this.router.events
      ...
      .subscribe((data: Data) => {
        // Thay đổi: Lấy trực tiếp title/description thay vì titleKey/descriptionKey
        const { title, description } = data['meta'] || {};
        
        // Bạn có thể sửa text mặc định dưới đây cho phù hợp project
        const pageTitle = title || 'Trang chủ'; 
        const pageDescription = description || 'Mô tả mặc định của ứng dụng';

        this.seoService.setTitle(`${environment.appName} - ${pageTitle}`);
        this.seoService.setDescription(pageDescription);
      });
  }
}